...
DO $$ 
BEGIN
  -- Drop all existing policies for certificate_requests
  ...
  DROP POLICY IF EXISTS "Anyone can view certificates" ON certificate_requests;
  DROP POLICY IF EXISTS "Users can request certificates" ON certificate_requests;
...
-- Create new policies
CREATE POLICY "Certificate view policy"
ON certificate_requests
FOR SELECT
TO authenticated
USING (true);

CREATE POLICY "Certificate request policy"
ON certificate_requests
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "Certificate admin policy"
ON certificate_requests
FOR ALL
TO authenticated
USING (auth.is_admin())
